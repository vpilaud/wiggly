...
\begin{proof}
  Let $T$ be the regular triangulation given by $f$.
  
  Assume $\simplex$ is the boundary complex of the convex hull of the points~$lambda(v) \, \b{R}(v)$ for all~$v\in V$, and consider $F$, a minimal non-face of $\simplex$.
  
  Choose a point $x\in \relint{F}$.
  Since $T$ is a triangulation covering $\conv{p_i}$, there is a unique $G\in T$ such that $x\in \relint{G}$.
  As $x\in \conv{F}, \conv{G}$, there are two convex combinations
  \[
    x = \sum_{p_i\in G} \lambda_i p_i = \sum_{q_j\in F} \mu_j q_j,
  \]
  with all $\lambda_i, \mu_j \geq 0$, unique up to scaling.
  Furthermore, no $\mu_j$ can be $0$ since $F$ is a minimal non-face, and no
  ...
  Finally, since $F$ is a non-face, it lies above $G$ in the height embedding given by $f$, so
  \[
  ...
  Since $\simplex$ has no boundary, its dual graph is $d$-regular, so in fact $T = \simplex$.
