...
Our construction of the wiggly fan is based on rigidity considerations, similar to those used in \cite{RoteSantosStreinu-polytope} to construct the fan (and the polytope) of pseudo-triangulations of a point set. In fact, for point sets in general position our construction restricts to the pone in that paper. Before looking at the case of wiggly arcs, let us briefly recall the basics of (infinitesimal) rigidity thepory.
...
R(P):=
\begin{pmatrix}
p_1-p_2 & p_2-p_1 & 0 & \dots & 0 & 0 \\
p_1-p_3 & 0 & p_3-p_1 & \dots & 0 & 0 \\
\vdots & \vdots & \vdots & & \vdots & \vdots \\
p_1-p_n & 0 & 0 & \dots & 0 & p_n-p_1 \\
0 & p_2-p_3 & p_3-p_2 & \dots & 0 & 0 \\
\vdots & \vdots & \vdots & & \vdots & \vdots \\
0 & 0 & 0 & \dots & p_{n-1}-p_n & p_n-p_{n-1}
\end{pmatrix}
\]
That is to say, we have a row vector $R(p,q)\in (\R^2)^n \cong (\R^2)^P$ associated to each pair of points $p,q\in P$; all entries in $R(p,q)$ are zero except for two blocks of size two: the block corresponding to $p$ has the vector $p-q$ and the block corresponding to $q$ has the block $q-p$.

Considering $R(P)$ as a linear map $(\R^2)^n\to \R^{\binom{n}{2}}$, it has the following interpretation: each $v=(v_1,\dots,v_n) \in (\R^2)^n$ is understood as a vector of velocities (or infinitesimal displacements) for the points in $P$, and the vector $R(P) v \in \R^{\binom{n}{2}}$ returns in the coordinate $\{i,j\} \in \R^{\binom{n}{2}}$ the value
\[
(p_i-p_j)\cdot (v_i-v_j),
\]
which equals the (infinitesimal) increase or decrease of the length of the segment $[p_i,q_i]$ under that motion.

The kernel of $R(P)$ are the infinitesimal motions that do not change any distance between points, that is, moving the whole point set as if it was a rigid block. If $P$ affinely spans $\R^2$ (whcih we always assume) then $R(P)$ has rank $2n-3$ and its three-dimensional kernel is generated by the vectors of the form $T_v:=(v,...,v)$ for an arbitrary $v\in \R^2$ plus the vector 
\[
P^\perp:=(p_1^\perp, \dots, p_n^\perp),
\]
where $(x,y)^\perp:=(-y,x)$ denotes the rotation of of $(x,y)$ by \(90^{\circ}\) in the positive direction.
The former represent global translations and the latter an (infinitesimal) rotation with center at the origin.
We denote this kernel $se(P)$%
\footnote{Although we only need to think of it as a linear space, our reason for the notation $se(P$ is that it equals the  Lie algebra of the Lie group $SE(2)$ of Euclidean motions in the plane, acting as global rigid motions on $P$. Observe that the kernel contains also the rotations with centers at points other than the origin, since the rotation with center at $o\in \R^2$ produces as infinitesimal motion of $P$ the vector $P^\perp - T_o$.
}

The dual map $\R^{\binom{n}{2}} \to (\R^2)^n$ also has a physical interpretation. We can think of each pair of points in $P$ as joined by a rod, and interpret each vector $w\in \R^{\binom{n}{2}}$ as expressing extension or compression stresses applied to the individual rods. Then the vector $R(P)^T w  \in (\R^2)^n$  equals $(f_1,\dots,f_n)$ where $f_i = \sum_j w_{ij} (p_i-p_j)$ is the resulting force at point $p_i$. The kernel of $R(P)^T$ hence represents the stresses that can be applied to the rods and produce no net force on each $p_i$. These are called \defn{equilibrium stresses}.

If we now consider a set of edges $E\subset \binom{n}{2}$, this defines a restricted matrix $R_E(P)$ whose kernel are the infinitesimal motions preserving the lengths of edges in $E$. Hence, $E$ is called (infinitesimally) \defn{rigid} (for the positions $P$) if $R_E(P)$ has full-rank, equal to $2n-3$. Similarly, the kernel of $R_E(P)^T$ represents the equilibrium stresses of the subgraph $E$, and $E$ is called \defn{stress-free} if that kernel is zero; that is, if $R_E(P)$ has rank $E$.%
...
To this vector we add a transversal part depending on the intermediate points $L \cup R$ along the arc.
...
We denote
\[
R_t(p,q;r):= (p-q)^\perp (\lambda_r \mathbbm{1}_p + \mu_r \mathbbm{1}_q - \mathbbm{1}_r).
\]
...
\begin{definition}\label{def:trv}
  ...
  \[
  R_t(\arc) = \sum_{r\in L} R_t(p,q;r) - \sum_{r\in R} R_t(p,q;r).
  \]
\end{definition}
...
\[
R_t(p,q;r) (v,\dots,v) =
(p-q)^\perp (\lambda_r \mathbbm{1}_p + \mu_r \mathbbm{1}_q - \mathbbm{1}_r) (v,\dots,v) =0
\]
since
\[
 (\lambda_r \mathbbm{1}_p + \mu_r \mathbbm{1}_q - \mathbbm{1}_r) (v,\dots,v) =
(\lambda_r  + \mu_r  - 1)  v  = 0. 
\]
For the rotation $P^\perp:=(p_1^\perp, \dots, p_n^\perp)$ we have
\begin{align*}
R_t(p,q;r) (p_1^\perp, \dots, p_n^\perp) 
&= (p-q)^\perp (\lambda_r \mathbbm{1}_p + \mu_r \mathbbm{1}_q - \mathbbm{1}_r) (p_1^\perp, \dots, p_n^\perp) =0 
\end{align*}
since
\[
 (\lambda_r \mathbbm{1}_p + \mu_r \mathbbm{1}_q - \mathbbm{1}_r) (p_1^\perp, \dots, p_n^\perp) =
\lambda_r p^\perp + \mu_r q^\perp - r^\perp =0
\]

In fact, the vectors $\{R_t(p,q;r)\}_{r\in L\cup R}$ are a basis for the linear space 
\[
\left\{ (p-q)^\perp \left(\sum_{r\in P} \lambda_r \mathbbm{1}_r\right) : \lambda_r=0 \text{ if } r \not\in L \cup R \cup \{p,q\} \right\} \cap se(P)^\perp.
\]
