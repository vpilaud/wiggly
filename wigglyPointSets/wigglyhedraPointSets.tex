...
\begin{proof}
  ...
  Let us now show that no infinitesimal motions lie in the kernel.
  Indeed, let \((v_p)_{p \in P}\) be a non-trivial infinitesimal motion in the kernel.
  By subtracting a translation and a rotation if necessary, we can assume that \(v_0 = v_1 = 0\).
  To show that nothing else lies in the kernel, we show that the row space of \(M^{\epsilon}(P)\) has dimension at least \(2n - 3\).
\fbox{TODO}
\end{proof}
