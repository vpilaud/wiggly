...
An \defn{oriented arc} from~$p$ to~$q$ is an isotopy class in the set of continuous maps~$\alpha : [0,1] \mapsto \R^2$ that satisfy~$\alpha(0) = p$, $\alpha(1) = q$ and~$\alpha(]0,1[) \subseteq \R^2 \ssm P$.
...
Let~$P$ be a point set, let $p,q\in P$, and let $S = P \cap\, ]p,q[$ be the subset of points along the open segment~$]p,q[$. A \defn{wiggly segment} is an arc from $p$ to $q$ obtained  perturbing the straightline arc $\alpha(t)\mapsto (1-t) p+t q$ so as to avoid each of the points of $S$ in the topologically simplest manner.%
\footnote{E.g., for each $r\in ]p,q[$ substitute a small half-circle with center at $r$ for a the portion of $\alpha$ going over $r$.} Wiggly segments with endpoints $\{p, q\}$ are in bijection with partitions of $S$ into a subset $L$ of points to the left of the (oriented) arc and a subset $R$ to the right.
...
\footnote{This information is captured by the linear matroid of rows of $\Rvector(p)$: rigid graphs are the spanning sets of rows, and stress-free graphs are the independent ones). Consequently, that matroid is called the (infinitisimal) rigidity matroid of $P$.}

\section{Wiggly rigidity}
\label{sec:w_rigidity}

The main result of  \cite{RoteSantosStreinu-polytope} is that when $P$ is in general position (and hence,  every wiggly segment is of the form $(p,q,\emptyset,\emptyset)$), the wiggly complex can be embedded in $(\R^2)^P$ associating to each  $(p,q)\in \binom{P}{2}$ the corresponding rigidity vector $\Rvector(p,q)$, and this procedure produces a complete (in its linear span) and polytopal fan. In order to generalise this to non-general position we need to modify the definition of rigidity vector associated to a wiggly arc.

Let $\arc=(p, q, L, R)$ be a \defn{partial wiggly arc}, by which we mean that $L$ and $R$ are disjoint subsets of $]p,q[ \cap P$, but not necessarily with $L \cup R = ]p,q[ \cap P$.
...
The coefficients  $\lambda_r$ and $\mu_r$ are the \defn{barycentric coordinates }of $r$ in $[p,q]$ and can alternatively be defined via
$
r-p = \mu_r  (q-p)$ and $q-r = \lambda_r  (q-p)$.
