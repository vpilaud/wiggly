...
The main result of  \cite{RoteSantosStreinu-polytope} is that when $P$ is in general position (and hence,  every wiggly segment is of the form $(p,q,\emptyset,\emptyset)$), the wiggly complex can be embedded into a complete (in its linear span) and polytopal fan~$(\R^2)^P$ by associating to each~$(p,q) \in \binom{P}{2}$ the corresponding rigidity vector~$\Rvector(p,q)$. In order to generalise this to non-general position we need to modify the definition of rigidity vector associated to a wiggly arc.
...
For each $r\in P$ let $\mathbbm{1}_r$ denote the matrix in $(\R^{2\times 2})^P \cong \R^{2\times 2n}$ having an identity $2\times 2$ matrix in the block of $r$ and zeroes elsewhere. Observe that for an $x\in \R^2$ we have that $x \mathbbm{1}_r \in (\R^2)^P$ equals the vector  that has $x$ in the $r$th coordinate and zeroes elsewhere. 
...
\[
\Rlong(\arc) := (p-q) (\mathbbm{1}_p  - \mathbbm{1}_q).
\]
\end{definition}
...
$w \left(\sum_{r\in P} \lambda_r \mathbbm{1}_r\right)$ with $w\in \R^2$ orthogonal to $q-p$ and $\lambda_r = 0$ if $r\not\in [p,q]$.
...
\[
\Rtrans(p,q;r)  = (q-p)^\perp (\mathbbm{1}_r - \lambda_r \mathbbm{1}_p - \mu_r \mathbbm{1}_q).
\]
\end{remark}
...
Writing $\Rtrans(p,q;r) = (q-p)^\perp (\mathbbm{1}_r - \lambda_r \mathbbm{1}_p - \mu_r \mathbbm{1}_q)$ in the fashion of Remark~\ref{rem:barycentric}, for infinitesimal translations we have that
\[
...
and
\[
(\mathbbm{1}_r - \lambda_r \mathbbm{1}_p - \mu_r \mathbbm{1}_q) (s^\perp)_{s\in P} =
...
\[
\Reps(p,q,L,R) + \Reps(p,q,R,L) = 2\Reps(p,q, \emptyset, \emptyset) =  2\Rvector(p,q).
\]
