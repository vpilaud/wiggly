...
\begin{proof}
  ...
  By inductively computing the entries of \(M^{\epslion}(P) \cdot \b(v)\) corresponding to segments between successive points on this line, we can check that \(v_{p_k}\) is orthogonal to The line \(p_1p_2\).
  ...
  Then the only contribution to \(M^{\epslion}(P)\cdot \b{v}\) comes from the transverse rigidity vectors, and it is a single dot product of \(v_k\) with a non-zero parallel vector.
