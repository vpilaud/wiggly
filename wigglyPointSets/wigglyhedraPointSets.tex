...
The rigidity vector of \(\arc\) is the combination of a longitudinal component that lies along \(\arc\), and a transversal component orthogonal to \(\arc\).
Only the endpoints of \(\arc\) contribute to the longitudinal component, while only the endpoints together with the waypoints contribute to the transversal component.
  ...
  When \(P\) is arbitrary, we show in ?? that a perturbation of the longitudinal rigidity vector by a small multiple of the transversal rigidity vector gives us a complete normal fan.

  All rigidity vectors lie in \((\R^2)^P \cong (\R^2)^n\).
  Given \(x \in \R^2\) and \(p \in P\), for convenience, we denote by \(x \mathbbm{1}_p\) or \(x \cdot \mathbbm{1}_p\) the vector in \((\R^2)^P\) that has \(x\) in the \(p\)th coordinate and zero elsewhere.
  In the following definitions, let \(P\) be a point set and \(\arc\) a wiggly segment with endpoints \(p\) and \(q\).
\begin{definition}
...
\begin{definition}
  ...
  The \defn{\(\epsilon\)-rigidity matrix} of \(P\) is a matrix \(M_P\) with rows indexed by the wiggly arcs on \(P\).
  For any \(\arc\), the row corresponding to \(\arc\) is \(R^{\epsilon}(\arc)\).
\end{definition}
% \begin{definition}
