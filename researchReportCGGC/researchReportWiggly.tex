...
\begin{definition}
	Let $\alpha$ be a wiggly arc. A \defn{truncation} of $\alpha$ at $i$ is defined in the following way:
	\begin{enumerate}
	...
	\end{enumerate}
	We denote the truncation of $\alpha$ around $i$ as $Tr_i(\alpha)$. Define the \defn{truncation decomposition} $D(\alpha)$ to be the formal sum
		\[
		D(\alpha) := \sum_{i=2}^{n-1} Tr_i(\alpha). %\in \Z[\widetilde {\wigglyArcs_P}].
		\]
\end{definition}
...
\begin{lemma}
	...
	For two incompatible arcs $\alpha, \alpha'$, let $\beta, \beta'$ be the upper and lower hulls. 
	\begin{itemize}
		\item If $\alpha, \alpha'$ are crossing arcs: 
		...
		Consequentially, 
		\[
		D(\alpha) + D(\alpha') - D(\beta)+D(\beta')=0.
		\]
		\item If $\alpha, \alpha'$ are non-pointed arcs: suppose $j+1$ is their common endpoint, then for $i \neq j$,
		\begin{align*}
		...
		\begin{align*}
			\begin{cases}
				Tr_i (\alpha) + Tr_i (\alpha') = (l,j) + (r,j),  \\  
				Tr_i (\beta) + Tr_i (\beta') = (u,j) + (d,j).
			\end{cases}%\quad \text{for } i = j;
		\end{align*}
        Consequentially,
        \[
   		2\pth{D(\alpha)+D(\alpha')} - \pth{D(\beta)+D(\beta')} = 2\pth{(l,j)+(r,j)} - \pth{(u,j)+(d,j)}.
		\]
	\end{itemize}
\end{lemma}

This ensures that when choosing the vector configuration $R$, we only need to be careful of the non-faces of type $\{(l,i),(r,i)\}$. More precisely,
%\medskip
\begin{corollary}
\label{c:extension}
If $\widetilde R:\widetilde {\wigglyArcs_P}\to \R^d$ is such that the only linear dependences among its values are:
\[
2\pth{\widetilde R((l,i))+\widetilde R((r,i))} - \pth{\widetilde R((u,i))+\widetilde R((d,i))}=0, \qquad \forall i\in\{2,\ldots,n-1\},
\]
then there exists $R:\wigglyArcs_P\to \R^d$ satisfying the condition of Theorem~\ref{thm:simplicial-complex-is-regular-triangulation}. 

More precisely, for every minimum non-face $F=\{\alpha,\alpha'\}$ of $\Delta$, let $\beta, \beta'$ be the upper and lower hulls. The set $\{\beta,\beta'\}$ is a face of $\wigglyArcs_P$, and
    \begin{itemize}
    \item if $\alpha$ and $\alpha'$ are crossing arcs: then $R(\alpha)+R(\alpha')=R(\beta)+R(\beta')$,
    \item if $\alpha$ and $\alpha'$ are non-pointed arcs: then $2R(\alpha)+2R(\alpha')=R(\beta)+R(\beta')$.
    \end{itemize}
\end{corollary}
\begin{proof}
Extend $\widetilde R$ linearly on $\Z[\widetilde \wigglyArcs_P]$, and define
\[
R:\wigglyArcs_P\to \R^d, \alpha\mapsto \widetilde R(D(\alpha)).
\]
The conclusion follows by Lemma~\ref{l:truncation-works} and linearity of $\widetilde R$. For the non-crossing case, the equality is immediate, but for the non-pointed case, we have to use the hypothesis on $\widetilde R$. 
\begin{comment}
Indeed, for every non-face $F=\{\alpha, \alpha'\}$ (with $\beta,\beta'$ the lower and upper hull of $\alpha,\alpha'$):
\begin{itemize}
...
\end{itemize}
\end{comment}
%The pair $\{\beta,\beta'\}$ is compatible and therefore a face of $\wigglyArcs_P$. 
\end{proof}
\begin{example}
The function $\widetilde R$ defined on $\widetilde {\wigglyArcs_P}$ as described in Definition~\ref{d:rigidity-vector} satisfies the condition of Corollary~\ref{c:extension}. 
\end{example}

It remains to specify a height function $f:\wigglyArcs_P\to \R$ before being able to apply Theorem~\ref{thm:simplicial-complex-is-regular-triangulation}.
