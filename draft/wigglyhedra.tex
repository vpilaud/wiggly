...
We now describe the \(t\)-structure induced by the full subcategory of \emph{linear complexes} of \(\mathcal{C}_m\); a variant of this construction is also described in~\cite[Section 2.5]{lic.que:21}.
...
\end{defprop}
The next statement, which is easy to check from the definitions, is mentioned in~\cite[Section 3.3]{bayer2011tour}.
...
The Khovanov--Seidel correspondence (explained in Section 4 of~\cite{kho.sei:02}) associates objects of \(\mathcal{C}_{n+1}\) (up to grading shift) to curves on a configuration of \(n+2\) marked points arranged in a straight line.
...
Let us first characterise wiggly arcs in terms of objects of \(\mathcal{C}_m\).
\begin{lemma}
  Every wiggly arc corresponds, up to grading shift, to an indecomposable spherical object in the heart \(\mathcal{A}_m\) of the standard \(t\)-structure on \(\mathcal{C}_m\).
\end{lemma}
\begin{proof}
  Let \(P^{\bullet}\) be the complex built from an arbitrary (not necessarily wiggly) arc.
  The results of~\cite{kho.sei:02} imply that \(P^{\bullet}\) is spherical and indecomposable; we now show that it belongs to \(\mathcal{A}_m\) up to grading shift if and only if the arc is a wiggly arc.
  
  Recall that the procedure to build up the complex of projective modules from an arc explicitly describes the differentials.    
...
\begin{proposition}\label{prop:hn-filtration}
  ...
  Let \(T\) be corresponding set of wiggly curves.
