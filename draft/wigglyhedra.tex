...
The triangulated category of interest will be the category \(\mathcal{C}_m = D^b(A_m\text{-mod})\) for a certain finite-dimensional algebra \(A_m\).
The algebra \(A_m\) is defined as the quotient of the path algebra of the quiver with \(m+1\) vertices depicted in~\cref{fig:am-quiver}.
\begin{figure}[h]
  \centering
  \begin{tikzcd}
    0 \arrow[bend left]{r}& 1 \arrow[bend left]{r} \arrow[bend left]{l}& 2 \arrow[bend left]{r}\arrow[bend left]{l}& \cdots \arrow[bend left]{r}\arrow[bend left]{l}& m \arrow[bend left]{l}
  \end{tikzcd}
...
Recall that the type \(A_m\) Dynkin graph is an undirected graph with \(m\) vertices and \(m-1\) edges; each edge connects successive pairs of points as shown in~\cref{fig:a5-dynkin}.
\begin{figure}[h]
  \centering
  \begin{tikzcd}
    \bullet \arrow[-]{r}& \bullet \arrow[-]{r}& \bullet \arrow[-]{r}& \bullet \arrow[-]{r}& \bullet
  \end{tikzcd}
...
\begin{figure}[h]
  \centering
  \begin{tikzcd}
    \bullet \arrow[bend left]{r}& \bullet \arrow[bend left]{r} \arrow[bend left]{l}& \bullet \arrow[bend left]{r}\arrow[bend left]{l}& \bullet \arrow[bend left]{r}\arrow[bend left]{l}& \bullet \arrow[bend left]{l}
  \end{tikzcd}
  \caption{The doubled quiver of the type \(A_5\) Dynkin graph.}
  \label{fig:a5-doubled}
\end{figure}
The \emph{zig-zag algebra} of an undirected graph is a finite-dimensional quotient of the path algebra of the doubled quiver of that graph (see~\cite[Section 3]{hue.kho:01} for the precise definition).

Let \(Z_m\) be the zig-zag algebra of the type \(A_m\) graph.
Let \(\mathcal{C}_m\) be \(K^b(Z-\text{proj})\), which is the bounded homotopy category of graded projective modules over \(Z_m\)
The category \(\mathcal{C}_m\) is triangulated.
...
The category \(\mathcal{C}_m\) is linear over a fixed field \(k\), triangulated, and 2-Calabi--Yau.
That is, for every pair of objects \(X\) and \(Y\) in \(\mathcal{C}_m\), we have
\[\Hom_{\mathcal{C}_m}(X,Y[d]) \cong \Hom_{\mathcal{C}_m}(Y, X[2-d]),\]
where \([1]\) denotes the triangulated shift.
\asilata{Update these definitions to the bigraded version, not the 2-CY version.}
Furthermore, \(\mathcal{C}_m\) is strongly generated by \(m\) objects denoted by \(P_1, \ldots, P_m\), which correspond to indecomposable projective modules over the zig-zag algebra.
These objects satisfy:
\begin{itemize}
\item \(\Hom(P_i,P_i[d]) = 0\) if \(d \neq 0, 2\);
\item \(\Hom(P_i,P_i[d]) = k\) if \(d = 0, 2\);
\item \(\Hom(P_i, P_{i\pm1}[d]) = 0\) if \(d \neq 1\);
\item \(\Hom(P_i, P_{i\pm1}[1]) = k\), and
\item \(\Hom(P_i, P_j[d]) = 0 \) otherwise.
\end{itemize}

Now consider \(n+2\) marked points on a disk, arranged on a straight horizontal line, and labelled \(0,1, \ldots, n+1\).
We will consider isotopy classes of curves whose endpoints lie on distinct marked points, and which do not cross themselves or any marked point in their interior.
For convenience, we also refer to such an isotopy class as a \emph{curve}.
The Khovanov--Seidel correspondence (explained in Section 4 of~\cite{kho.sei:02}) associates objects of \(\mathcal{C}_{n+1}\) to curves on a configuration of \(n+2\) marked points arranged in a straight line.

In particular, the line segment joining point \(i\) to point \(i+1\) is sent to the object \(P_{i+1}\) in the category \(\mathcal{C}_{n+1}\).
It is easy to check that the (minimal) intersection numbers between any two such segments counts the number of morphisms between the associated objects.
...
\asilata{Now figure out what to say about \(t\)-structures, namely that wiggly arcs are exactly those whose associated complexes lie in the heart of the standard \(t\)-structure.}
