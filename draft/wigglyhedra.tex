...
The category of interest is a subcategory of \(D^b(A_m\text{-mod})\) for a certain finite-dimensional algebra \(A_m\).
...
The zigzag algebra \(A_m\) is the quotient of \(kQ_m\) by the following relations:
\begin{itemize}
\item set \((i|j|l) = 0\) whenever \(i \neq l\);
\item set \((i|j|i) = (i|l|i)\) for every \(j\) and \(l\) such that there are arrows between \(i\) and \(j\) and between \(i\) and \(l\).
\end{itemize}
See~\cite[Section 3]{hue.kho:01} for further details, and~\cite[Remark 6.6]{bap.deo.lic:20} for a more detailed discussion.
...
Set \(\mathcal{C}_m\) to be the smallest triangulated subcategory of \(D^b(A_m\text{-mod})\) containing all the objects \(P_i\langle j \rangle\) (see, e.g.,~\cite[Section 2.3.3]{bap.deo.lic:22} for further discussion).
...
We consider isotopy classes of curves whose endpoints lie on distinct marked points, and which do not cross themselves or any marked point in their interior.
