...
These two perspectives enable us to endow this combinatorial families with additional structures : the wiggly pseudotriangulations are actually the facets of the wiggly complex, while the wiggly permutations are actually the elements of the wiggly lattice.
...
We start with wiggly pseudodissections of points on a line.
...
%
\begin{figure}[b]
\centerline{\includegraphics[scale=1.3]{pseudodissections}}
...
\begin{figure}
\centering
\includegraphics[scale=1]{wigglyFlipGraph}
...
We close this section by the following conjecture (checked computationally up to~$n = 4$).
...
Consider a closed simplicial pseudomanifold~$\Delta$ with ground set~$V$ and a set of vectors~$(\b{r}_v)_{v \in V}$ of~$\R^d$, and define~$\b{R}_\triangle \eqdef \set{\b{r}_v}{v \in \triangle}$ for any~$\triangle \in \Delta$.
...
We now define two families of vectors.
%
\begin{figure}
...
\begin{itemize}
\item the intersection of the halfspaces~$\set{\b{x} \in \HH}{\dotprod{\b{g}(\alpha)}{\b{x}} \le \nu(\alpha)}$ for all internal wiggly arcs~$\alpha$,
\item the convex hull of the points~$\sum_{\alpha \in T} \kappa(\alpha) \, \b{c}(\alpha, T)$ for all wiggly pseudotriangulations~$T$.
\end{itemize}
\end{theorem}
