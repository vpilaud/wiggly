...
Finally, recall from~\cref{lem:linearDependences} that the \(g\)-vectors of the arcs \(\alpha\), \(\alpha'\), \(\beta\), and \(\beta'\) satisfy a linear dependence, depending on their type.
...
\end{proof}
Note that if we write down the relations in the Grothendieck group of our category that correspond to the extensions from each case of~\cref{prop:extensions}, then we obtain precisely the linear dependences that appear between the \(g\)-vectors in~\cref{lem:linearDependences}.
...
\item if \(\alpha\) and \(\alpha'\) are crossing, then
  \([X] + [X'] = [Y] + [Y'].\)
\end{itemize}
This suggests that there should be a way to construct the \(g\)-vectors categorically in such a way that the \(g\)-vectors also map to the Grothendieck group of the category.
At the moment, we do not have a concise construction of the \(g\)-vectors purely categorically, and it would be interesting to find this.
