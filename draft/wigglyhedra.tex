...
\begin{proof}
  ...
  \subsection*{\(\Phi(T)\) is a well-defined wiggly permutation for each wiggly pseudotriangulation \(T\).}
  ...
  The other case (when \(2j\) appears before \(2j-1\) in \(\sigma\)) is similar.
  
  \subsection*{\(\Psi(\sigma)\) is a wiggly pseudotriangulation for each wiggly permutation \(\sigma\).}
  ...
  \subsection*{The maps \(\Phi\) and \(\Psi\) are inverse bijections that induce the directed graph isomorphism claimed in the statement.}
  \asilata{Todo.}
\item 
...
\item $\b{c}(\alpha, T)_u = 1/2$ if~$\bar u = \bar v$, or if~$\bar u$ and the wiggly arcs incident to~$\bar u$ are on the same side of~$\bar u$, and $\b{c}(\alpha, T)_{2\bar u-1} = \b{c}(\alpha, T)_{2\bar u} = 1/4$ otherwise,
