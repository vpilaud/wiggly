...
\begin{definition}
  The zigzag algebra \(A_m\) corresponding to the quiver \(Q_m\) depicted in~\cref{fig:am-quiver} is the quotient of \(kQ_m\) by the following relations:
  \begin{itemize}
...
\begin{remark}
  ...
  This is an ``un-signed'' version of the zigzag algebra.
  It is technically better to use a ``signed'' version of the zigzag algebra, and the comparison between the two is discussed in~\cite[Remark 6.6]{bap.deo.lic:20}.
...
We now describe the full subcategory of \emph{linear complexes} of \(\mathcal{C}_m\).
...
Recall that an object of the form \(M_i[h]\) is placed in homological degree \(-h\);
since the differential \(d\) raises the homological degree by one, it \emph{lowers} the homological shift by one.
That is, we have
\[d \colon M_i\langle g \rangle[h] \to \bigoplus M_{i'}\langle g' \rangle[h-1].\]
...
\begin{definition}\label{def:linear-complex}
  ...
  \[P^{\bullet} = \bigoplus M_i\langle g \rangle[h]\]
  together with an internal differential \(d \colon P^{\bullet} \to P^{\bullet + 1}\).
  We say that \(P^{\bullet}\) has \emph{level \(\ell\)} for some \(\ell \in \mathbb{Z}\) if for each bigraded piece \(M_i \langle g \rangle[h]\) of \(P^{\bullet}\), we have \(g + h = \ell\).
  We say that \(P^{\bullet}\) is \emph{linear} if \(P^{\bullet}\) has level \(\ell = 0\).

\end{definition}
Let \(\mathcal{A}_m\) be the smallest full subcategory of \(\mathcal{C}_m\) that is closed under isomorphism, which contains all the linear complexes.
\begin{proposition}\label{prop:standard-heart}
  The category \(\mathcal{A}_m\) is abelian.
  Furthermore, it is the heart of a bounded \(t\)-structure on \(\mathcal{C}_m\), which we call the \emph{standard \(t\)-structure} on \(\mathcal{C}_m\).
\end{proposition}
\asilata{Find a reference for this and possibly a cleaner statement.}

Also recall the following statements for future use.
\begin{defprop}\label{defprop:cohomology}
  Let \((\mathcal{C}^{\le 0}, \mathcal{C}^{\geq 0})\) be a bounded \(t\)-structure on a triangulated category \(\mathcal{C}\) with heart \(\mathcal{A} = \mathcal{C}^{\leq 0} \cap \mathcal{C}^{\geq 0}\).
  For each integer \(i\), we have the \(i\)th truncation functors
  \[\tau^{\leq i} \colon \mathcal{C} \to \mathcal{C}^{\leq i}\]
  and
  \[\tau^{\geq i} \colon \mathcal{C} \to \mathcal{C}^{\geq i},\]
  which are left and right adjoint respectively\asilata{Check left vs right.} to the inclusion functors \(\mathcal{C}^{\leq 0} \to \mathcal{C}\) and \(\mathcal{C}^{\geq 0} \to \mathcal{C}\).
  The \(i\)th cohomology functor is defined to be the composition \(\tau^{\leq i} \circ \tau^{\geq i}\).
  It is equal to the composition \(\tau^{\geq i} \circ \tau^{\leq i}\), and maps an object of \(\mathcal{C}\) to an object of \(\mathcal{A}\).
\end{defprop}
In the presence of a bounded \(t\)-structure, every object of \(\mathcal{C}\) has a canonical filtration whose factors are shifts of its cohomology objects.
\begin{defprop}\label{defprop:cohomology-filtration}
  ...
  \asilata{Find references for cohomology and cohomology filtration.}
  Then \(X\) has a unique filtration of the form
  \[
...
For convenience, we refer to such an isotopy class as an \emph{arc}.

The Khovanov--Seidel correspondence (explained in Section 4 of~\cite{kho.sei:02}) associates objects of \(\mathcal{C}_{n+1}\) (up to grading\asilata{Should this say homological?} shift) to curves on a configuration of \(n+2\) marked points arranged in a straight line.

In particular, the line segment joining point \(i\) to point \(i+1\) is sent to the object \(P_{i+1}\) in the category \(\mathcal{C}_{n+1}\).
It is easy to check that the (minimal) intersection numbers between any two such segments counts the number of morphisms\asilata{Discuss (or choose to ignore) grading.} between the associated objects.
In particular, the self-intersection number of a single segment is computed to be \(2\), as the second copy of the segment can be slightly deformed to be disjoint from the first copy except at the two endpoints.
In fact, it is true for any pair of curves that its minimal intersection number is the number of morphisms between the associated objects (\cite[Lemma 4.11]{kho.sei:02}).
...
As such, this section is somewhat dual to the previous section --- our criterion determines whether a pair of wiggly arcs can belong together in a single wiggly pseudotriangulation, whereas the previous section generated a wiggly pseudotriangulation given any object of the category.
\begin{proposition}
  ...
  Then \(\alpha\) and \(\alpha'\) are compatible in the sense of~\cref{def:compatible} if and only if \(\Ext^1(X\langle g \rangle, X\langle g' \rangle) = 0\) for any two integers \(g\) and \(g'\).
\end{proposition}
\begin{remark}
...
\end{remark}
\begin{remark}
  As stated, this proposition is not expected to be true in greater generality.
  However, we expect to see similar constraints on compatibility, which we hope to turn to in forthcoming work.
  \asilata{Decide what to say here, because the forthcoming work is far from complete, although I do have counterexamples in type \(D_4\). Discussing how to interpret these counterexamples may not involve too much more work?}
\end{remark}
\begin{question}
