...
If~$\alpha$ and~$\alpha'$ are crossing and there is~$k \in [n-1]$ with $k \in (A \cap B') \cup (\{i\} \cap B') \cup (A \cap \{i'\})$ and~$k+1 \in (A' \cap B) \cup (\{j'\} \cap B) \cup (A' \cap \{j\})$, then we have
\begin{gather*}
...
\begin{definition}
The \defn{$\delta$-gon} is a convex $(n+2)$-gon with a point at abscissa~$i$ and ordinate of the sign of~$\delta_i$  for all~$0 \le i \le n+1$
A \defn{$\delta$-triangulation} is a set of ordered pairs~$(i,j)$ which are the internal diagonals of a triangulation of the $\delta$-gon.
...
\begin{definition}
A \defn{$\delta$-permutation} is a permutation of~$[n]$ which avoids~$\cdots ik \cdots j \cdots$ with~$i < j < k$ and~$\delta_j = -$, and~$\cdots j \cdots ik \cdots$ with~$i < j < k$ and~$\delta_j = +$.
...
The \defn{$\delta$-wiggly arcs} are the arcs~$(0, j, {[1,j[}, \varnothing)$ for~$\delta_j > 0$ and~$(0, j, \varnothing, {[1,j[})$ for~$\delta_j < 0$.
