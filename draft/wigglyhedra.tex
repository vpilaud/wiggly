...
\asilata{I can come up with a possible categorical pathway towards the wiggly flip graph for arbitrary finite Coxeter groups, related to~\cref{prop:hn-filtration}. But I have no idea how to construct the polytope!}
\end{remark}
...
For any integer \(N\), let \(\mathcal{C}_m^{\le N} \subset \mathcal{C}_m\) be the smallest full subcategory of \(\mathcal{C}_m\) closed under isomorphism, containing all complexes of level \(\leq N\).
For any integer \(N\), let \(\mathcal{C}_m^{\geq N} \subset \mathcal{C}_m\) be the smallest full subcategory of \(\mathcal{C}_m\) closed under isomorphism, containing all complexes of level \(\geq N\).
Observe that if \(P^{\cdot}\) has level \(\ell\), then \(P^{\cdot}[1]\) has level \(\ell + 1\), and that \(P^{\cdot}[-1]\) has level \(\ell - 1\).
\asilata{The directions of the shifts are a mess.}
