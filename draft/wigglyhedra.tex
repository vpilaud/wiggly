...
See~\cite[Section 3]{hue.kho:01} for further details on the definition, and~\cite[Remark 6.6]{bap.deo.lic:20} for more discussion.
...
Set \(\mathcal{C}_m\) to be the smallest triangulated subcategory of \(D^b(A_m\text{-mod})\) containing all the objects \(P_i\langle j \rangle\) (see, e.g.,~\cite[Section 2.3.3]{bap.deo.lic:22} for further discussion on the construction).

The category \(\mathcal{C}_m\) is \(k\)-linear and triangulated, where \([1]\) denotes the triangulated shift.
For \(X, Y \in \mathcal{C}_m\), let \(\Hom^{i,j}(X,Y)\) denote the space of morphisms in \(\mathcal{C}_m\) of homological degree \(i\) and internal degree \(j\).
In other words, we have
\[\Hom^{i,j}(X,Y) = \Hom(X,Y\langle i \rangle[j]).\]
Furthermore, for any integers \(a\) and \(b\), we have
\[\Hom^{i,j}(X\langle a \rangle[b], Y\langle a \rangle[b]) = \Hom^{i,j}(X,Y).\]

\begin{remark}\label{rem:generating-morphisms}
  The generating objects \(P_i\) (and hence their graded and homological shifts) satisfy the following:
  \begin{itemize}
  \item \(\Hom^{a,b}(P_i,P_i) = 0\) if \(b \neq 0\) and \(a \neq 0, 2\);
  \item \(\Hom^{a,0}(P_i,P_i) = k\) if \(a = 0, 2\);
  \item \(\Hom^{a,b}(P_i, P_{i\pm1}) = 0\) if \(b \neq 0\) and \(a \neq 1\);
  \item \(\Hom^{1,0}(P_i, P_{i\pm1}) = k\), and
  \item \(\Hom^{a,b}(P_i, P_j) = 0 \) otherwise.
  \end{itemize}
\end{remark}

The category \(\mathcal{C}_m\) has a standard bounded \(t\)-structure, whose heart consists of the \emph{linear complexes}.
\asilata{Say more about linear complexes.}

\subsection{The Khovanov--Seidel curve model}
In this section we briefly recall the procedure to recover an object of \(\mathcal{C}_{n+1}\) from a wiggly arc on \(n+2\) points on a line.
...
\begin{lemma}
Every wiggly arc corresponds to an object in the heart of the standard \(t\)-structure.  
\end{lemma}
\begin{proof}
  The procedure to build up the complex of projective modules from a curve explicitly describes the differentials.
  Each component of these differentials is a map from a shift of \(P_i\) to a shift of \(P_j\).
  It can be seen from~\cref{rem:generating-morphisms} that any component that maps from \(P_i\) to \(P_j\) for \(i \neq j\) keeps the complex linear; it is only when we have a looping map from \(P_i\) to \(P_i\langle 2 \rangle\) that we move out of the standard heart.
...
\subsection{Categorical conditions on compatibility of arcs}
...
\begin{proposition}
  ...
  \[
    \begin{tikzcd}[column sep=0.3cm]
      0 = X_0 \arrow{rr} && X_1 \arrow{dl} \arrow{rr} && \cdots \arrow{rr} \arrow{dl} && X_{k-1} \arrow{rr} \arrow{dl} && X_k = X \arrow{dl}\\
      & H^1(X) \arrow[dashed]{ul}{+1} && H^2(X) \arrow[dashed]{ul}{+1} && \cdots \arrow[dashed]{ul} &&H^k(X) \arrow[dashed]{ul}{+1}
    \end{tikzcd}
  \]
  Consider the set of objects \(\{H^j(X)\}\) taken without multiplicity, and let \(T\) be corresponding set of wiggly curves.
