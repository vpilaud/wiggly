...
\begin{proof}
\vincent{I disagree with some points in this proof, so I put an alternative one below...}
...
Finally, one can check using~\cref{lem:wigglyCoverRelation} that increasing flips in triangulations correspond precisely to edges in the wiggly lattice.
\vincent{Indeed, we need more here. Todo.}
\end{proof}
...
\vincent{Move that to the corresponding sections, and delete this section.}
%
\begin{figure}
...
\vincent{Integrate that to the previous section}
\begin{enumerate}
...
\vincent{What about this remark instead?}
\begin{remark}\label{rem:generating-morphisms}
  It is easy to check that the generating objects \(P_i\) (and hence their graded and homological shifts) satisfy \(\Hom^{0,0}(P_i,P_i) = \Hom^{1,0}(P_i,P_i) = \Hom^{1,0}(P_i,P_{i\pm1}) = k\) and \(\Hom^{g,h}(P_i, P_j) = 0\) in all other cases.
\end{remark}
...
In fact, it is true for any pair of curves that its minimal intersection number is the number of morphisms between the associated objects~\cite[Lem.~4.11]{kho.sei:02}.
...
\begin{proof}
  ...
  \(T\) cannot contain both external edges because otherwise, the curve \(\alpha\) will not be able to have a second endpoint.\asilata{I am struggling to explain this, even though it should be obvious.}
\end{proof}
...
\begin{remark}
  ...
  For instance, it does not hold for the zigzag algebra of Dynkin type~$D_4$.
