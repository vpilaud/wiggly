...
The following definitions are illustrated~in~\cref{fig:pseudodissections}.\asilata{It looks like the degree of the green cell is 6 in the figure, not 8.}
...
\]
\asilata[inline]{I think the correct definition of \(\alpha(\sigma,k)\) is as follows.
  \[\alpha(\sigma,k) = \big(i,j, {]i,j[} \cap \tau(k), {]i,j[} \ssm \tau(k)\big),\]
  where
  \[\tau(k) = \{l \in [n] \mid 2l - 1 \in \sigma([k])\text{ and } 2l \in \sigma([k])\}.\]
}
...
%We first have to prove that~$\Phi$ and~$\Psi$ are well-defined.
  \vincent{todo}
  Let us first prove that for any wiggly permutation \(\sigma\), the set \(\Psi(\sigma)\) is a wiggly pseudotriangulation.
  ...
  First suppose that \(k \leq k'\) and \(\alpha(\sigma,k) = \alpha(\sigma,k') = (i,j,A,B)\).
  It is easy to see that for any \(l \in [n]\), we have \(2l \in \sigma([k']) \ssm \sigma([k])\) if and only if \(2l -1 \in \sigma([k']) \ssm \sigma([k])\).
  If \(2l \in \sigma([k']) \ssm \sigma([k])\); equivalently \(2l - 1 \in \sigma([k']) \ssm \sigma([k])\), it is easy to check that
  \[i < l < j.\]
  \asilata[inline]{Fix definitions to check distinctness.}
  ...
  \asilata[inline]{Fix definitions to check non-crossing-ness.}  
...
\[
\dotprod{\b{c}(\alpha, T)}{\b{\omega}} = \sum_{i \in [n]} 4ni \dotprod{-\b{c}(\alpha, T)}{\b{e}_{2i-1} + \b{e}_{2i}} + \sum_{j \in [2n]} j \dotprod{-\b{c}(\alpha, T)}{\b{e}_j} = 1/2 > 0.
\]
