...
\resume{Motivés par la théorie catégorique des représentations, nous étudions le slalomèdre. Son complexe de bord encode les dissections slalomantes (ensembles deux-à-deux pointés et sans croisement d'arcs slalomant autour de $n+2$ points sur une ligne). Son graphe est le diagramme de Hasse du sous-treillis de l'ordre faible induit par les permutations slalomantes (permutations de $[2n]$ évitant~$2j-1 \cdots i \cdots 2j$ pour~$i < 2j-1$ et~$2j \cdots k \cdots 2j-1$ pour~$k > 2j$). Il contient des copies de tous les treillis Cambriens et associaèdres Cambriens de type~$A_n$.}
...
Its graph can be oriented to obtain the Hasse diagram of the wiggly lattice, and it contains copies of all type~$A_n$ Cambrian lattice as faces (\cref{sec:CambrianConsiderations}).
